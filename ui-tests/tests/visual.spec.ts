 ...
 * not JupyterLab chrome) in light + dark themes, masking the per-message
 * timestamps (the only dynamic content). Runs against the model-free stub
 * agent, so it's deterministic and needs no API key.
 *
 ...
 * browser versions). Regenerate via the ui-tests workflow's update path.
 */
